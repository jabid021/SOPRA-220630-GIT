			...
			<c:forEach items="${evenements}" var="e">
				<tr>
					...
					<c:choose>
							...
							<c:when test="${e.getClass().getSimpleName().equals('Competition')}"><td>${e.sport}</td><td>${e.gagnant.id}</td></c:when>
							<c:otherwise><td>  /  </td><td>  /  </td></c:otherwise>
						</c:choose>
					<td><a href="evenements?id=${e.id}"><button class="btn btn-warning">Modifier</button></a>
						<form action="evenement.java" method="post" >
							<input type="hidden" name="tache" value="modifier"> <input
								type="hidden" name="id" value="${e.id}"> <input type="submit"
...
<div id="formAjout" align="center">
	...
	<form action="evenements" method="post">
		<input type="hidden" name="tache" value="ajouter"> Libellé : <input
			required name="libelle" type="text" placeholder="Libelle evenement"><br>
		<br> Date de début : <input required name="dateDebut" type="date">
		<br>
		<br> Heure de début : <input required name="heureDebut"
			type="time"><br>
		<br> Date fin : <input required name="dateFin" type="date"><br>
		<br> Heure fin : <input required name="heureFin" type="time"><br>
		<br> Prix : <input name="prix" required type="number"
			placeholder="Prix" min="0"><br>
		<br>
		...
		<fieldset>
			...
			Numero : <input name="numero" type="text"  required> Voie : <input
				...
				name="cp" type="text"  required> Ville : <input
				name="ville" type="text" required>
		</fieldset>
	...
	<div id="choixSport">	<br> Choix du Sport : <select name="sport">
			<option>Judo</option>
			<option>Foot</option>
			<option>LoL</option>
			<option>Autre</option>
