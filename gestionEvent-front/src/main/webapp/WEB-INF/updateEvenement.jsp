...
<body>
	...
	<form action="evenements" method="post">
		<input type="hidden" name="tache" value="modifier"> <input
			type="hidden" name="id" value="${evenement.id}"> Libellé : <input
		...
		<fieldset>
			...
			Numero : <input name="numero" type="text"
				value="${evenement.adresse.numero}"> Voie : <input
				name="voie" type="text" value="${evenement.adresse.voie}">
		...
		<br> Type d'évenement :
		<c:if
			test="${evenement.getClass().getSimpleName().equals('Festival')}">
			<input checked name="typeEvenement" value="festival" id="Efestival"
				type="radio">Festival </c:if>

		<c:if
			test="${evenement.getClass().getSimpleName().equals('Competition')}">
			<input checked name="typeEvenement" value="competition"
				id="Ecompetition" type="radio">Compétition
				...
				<br> Choix du Sport : <select name="sport">
					...
					<option value="-1">Pas de gagnant</option>
					<c:forEach items="${participants}" var="p">
						<option <c:if test="${p.id==evenement.gagnant.id}">selected</c:if>  value="${p.id}">${p.nom}</option>
					</c:forEach>
		...
		<br> <input type="submit" class="btn btnViolet" value="Modifier">
		<a href="evenements"><button class="btn btn-dark" type="button">Annuler</button></a>
	</form>
</body>
<script>
